...
// Using global class definitions - using existing classes if already loaded or creating dummies
// Using variables to avoid redeclaration errors
var DummyAnalysisModule = class {
    constructor() { }
    initialize() { return Promise.resolve(true); }
    connectMicrophone() { return Promise.resolve(false); }
    startSimulation() { }
    stopSimulation() { }
    analyze() { }
    dispose() { }
    get analysisData() { return { volume: 0, normalizedBands: { bass: 0, mid: 0, high: 0 } }; }
};

var DummyEffectsModule = class {
    constructor() { }
    processAudio(data, params) { return params; }
    getPulseIntensity() { return 0; }
    isParameterActive() { return false; }
    updateMappings() { }
};
...
class SoundInterface {
    constructor(options = {}) {
        ...
        this.analysisModule = window.AnalysisModule ?
            new window.AnalysisModule({
                onStatusUpdate: (message) => this._handleStatusUpdate(message)
            }) :
            new DummyAnalysisModule();

        this.effectsModule = window.EffectsModule ?
            new window.EffectsModule() :
            new DummyEffectsModule();
...
// Make SoundInterface globally available
window.SoundInterface = SoundInterface;