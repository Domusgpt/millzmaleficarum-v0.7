 ...
 */
// Using global versions of these classes
// HypercubeCore, ShaderManager, GeometryManager, ProjectionManager, and SoundInterface
// are made available globally through their respective files

document.addEventListener('DOMContentLoaded', () => {
    const canvas = document.getElementById('hypercube-canvas');
    const statusDiv = document.getElementById('status');

    // Return early if canvas doesn't exist, without trying to use statusDiv
    if (!canvas) {
      console.warn("Error: Hypercube canvas not found.");
      return;
    }
    ...
    async function setupAudio() {
        try {
            ...
            console.log("Mic OK - Visualizer Active"); 
    ...
    function calculateAudioLevels() {
        ...
        try {
            ...
            if (!hasAudioSignal) {
                ...
                if (Math.random() < 0.01) {
                    console.log(`Simulated Note: ${analysisData.pitch.note}${analysisData.pitch.octave}`);
                }
    ...
    async function initialize() {
        try {
            ...
            console.log("Creating visualization core...");
